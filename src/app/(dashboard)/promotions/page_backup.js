...
export default function PromotionsPage() {
  ...
  const handleConsultAI = async () => {
    ...
    try {
      ...
      setSuggestion(res);
      setActiveStep(3);
      success(t('promo.generating_ai'));
    } catch (e) {
      console.error(e);
      error(e.response?.data?.error || t('common.error'));
    } finally {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h3 className="text-3xl font-black tracking-tight text-slate-900 uppercase italic flex items-center gap-3">
             {t('nav.promotions')} <Sparkles className="w-8 h-8 text-emerald-500" />
          </h3>
          <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-1">{t('promo.subtitle')}</p>
        </div>

        {/* Stepper Navigation */}
        <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200">
           {[
             { id: 1, label: t('promo.select') },
             { id: 2, label: t('promo.strategy') },
             { id: 3, label: t('promo.results') }
           ].map(s => (
             <button
               key={s.id}
          ...
          <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-12 gap-6 animate-in fade-in duration-500">
            <div className="md:col-span-5 lg:col-span-3 space-y-4">
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                 <Label className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-4 block">{t('promo.pick_items')}</Label>
            ...
            <div className="md:col-span-7 lg:col-span-9 flex flex-col justify-between">
                     ...
                     <div className="flex items-center justify-between">
                        ...
                        {selectedItems.length > 0 && (
                           <div className="px-3 py-1 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest italic leading-none">
                              {selectedItems.length} {t('common.item')}s
                           </div>
                        )}
                     </div>
               ...
               {selectedItems.length >= 2 && (
                  <div className="mt-8 flex justify-end">
                     ...
                     >
                        {t('common.next').toUpperCase()}: {t('promo.strategy').toUpperCase()} <ArrowRight className="w-4 h-4 ml-1" />
                     </Button>
                  </div>
               )}
            </div>
          ...
          <div className="lg:col-span-12 max-w-4xl mx-auto w-full animate-in slide-in-from-right-4 duration-500 pb-10">
             <div className="bg-white p-8 md:p-10 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                   <div className="space-y-8">
                      <div className="space-y-2">
                         <h3 className="text-xl font-black text-slate-900 uppercase italic">{t('promo.market_context')}</h3>
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('insights.subtitle')}</p>
                      </div>
                      ...
                      <div className="space-y-6">
                         ...
                         <div className="space-y-3">
                            <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{t('promo.inventory')}</Label>
                         ...
                         <div className="space-y-3">
                            <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{t('promo.benchmark')}</Label>
                   ...
                   <div className="space-y-8">
                      <div className="space-y-2">
                         <h3 className="text-xl font-black text-slate-900 uppercase italic">{t('promo.focus_strategy')}</h3>
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('insights.subtitle')}</p>
                      </div>

                      <div className="space-y-4">
                         {[
                           { id: "LOSS_LEADER", icon: Zap, label: t('promo.loss_leader'), desc: t('promo.traffic_desc') },
                           { id: "VOLUME_BOOSTER", icon: TrendingUp, label: t('promo.volume_booster'), desc: t('promo.transaction_desc') },
                           { id: "PROFIT_MAXIMIZER", icon: Target, label: t('promo.profit_maximizer'), desc: t('promo.margin_desc') }
                         ].map(strat => (
                           <button
                             key={strat.id}
                         ...
                         <div className="space-y-3 pt-2">
                            <Label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">{t('promo.target_margin')}</Label>
                ...
                <div className="mt-12 flex flex-col md:flex-row gap-4">
                   <Button onClick={() => setActiveStep(1)} variant="ghost" className="h-14 px-10 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400">{t('common.back')}</Button>
                   <Button disabled={isConsulting} onClick={handleConsultAI} className="flex-1 h-16 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black uppercase text-xs tracking-widest shadow-xl shadow-emerald-500/10 active:scale-95 transition-all">
                      {isConsulting ? <Loader2 className="w-6 h-6 animate-spin text-white" /> : t('promo.generate')}
                   </Button>
                </div>
             </div>
          </div>
              ...
              <div className="p-10 bg-slate-900 rounded-[2.5rem] text-white space-y-10 shadow-2xl relative overflow-hidden group">
                 <div className="flex flex-col md:flex-row justify-between items-start gap-8 relative z-10">
                    ...
                    <div className="md:w-72 p-5 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm">
                       <p className="text-[9px] font-black text-emerald-500/60 uppercase tracking-widest mb-3">{t('promo.ai_reasoning')}</p>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-5 relative z-10">
                    <div className="p-6 bg-white/5 rounded-3xl border border-white/10">
                       <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('kitchen.master_database')}</p>
                    ...
                    <div className="p-6 bg-white/5 rounded-3xl border border-white/10">
                       <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('promo.estimated_profit')}</p>
                 ...
                 <div className="pt-8 border-t border-white/10 flex flex-col lg:flex-row gap-8 justify-between items-center relative z-10">
                    <div className="flex gap-12 overflow-x-auto pb-2 scrollbar-hide max-w-full">
                       <div>
                          <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-0.5">{t('kitchen.cost').toUpperCase()}</span>
                    ...
                    <div className="flex gap-3 w-full lg:w-auto">
                       <Button onClick={() => setActiveStep(2)} variant="ghost" className="h-14 px-6 rounded-xl text-slate-500 hover:text-white font-black uppercase text-[9px]">{t('common.back')}</Button>
                       <Button onClick={handlePushToPOS} disabled={isSaving} className="flex-1 lg:flex-none h-16 px-10 rounded-2xl bg-white text-slate-900 hover:bg-emerald-50 font-black uppercase text-xs tracking-widest transition-all">
                          {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : t('promo.publish')}
                       </Button>
                    </div>
                 </div>
              </div>
